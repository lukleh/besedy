...
test.describe("API Security Boundaries @security", () => {
  ...
  test("LISTENER cannot browse the recordings API", async ({ page }) => {
    await loginAs(page, "listener");
  ...
  test("LISTENER cannot browse recording filter options", async ({ page }) => {
    await loginAs(page, "listener");
  ...
  test("host cannot grant another host via API", async ({ page }) => {
    ...
    // the protected-role policy check.
    const searchAndPickFirst = async (
      query: string
    ): Promise<string | null> => {
    ...
    expect(
      targetUserId,
      "expected a searchable target user for OWNER grant test"
    ).not.toBeNull();

    // A host cannot grant another role carrying manage_access.
    ...
    // Only catalog administrators can grant protected roles.
    expect(response.status()).toBe(403);
    const body = await response.json();
    expect(body.error).toMatch(/admin|owner/i);
  });

  test("Admin can grant catalog-admin access via API", async ({
    page,
  }, testInfo) => {
    ...
    // A system admin resolves as a catalog administrator.
    const response = await page.request.post(
      `/api/catalogs/${TEST_CATALOG_ID}/access`,
      {
        data: {
    ...
    // Catalog administrators can grant any catalog role.
    ...
    // The DELETE endpoint is a soft-revoke, leaving a protected revoked row
    // behind. Non-admin sibling tests searching for this user would then
    // get zero results (the protected grant is filtered out for hosts, and
