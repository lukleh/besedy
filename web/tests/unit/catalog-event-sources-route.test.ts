...
describe("catalog event sources route", () => {
  ...
  it("returns 403 when the user cannot manage event sources", async () => {
    const response = await listEventSources(
      new NextRequest(`http://localhost/api/catalogs/${catalogId}/events/${eventId}/sources`),
