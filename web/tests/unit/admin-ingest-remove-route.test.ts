...
function workerRequest(intakeId: string, body: unknown) {
  return new NextRequest(
    `http://localhost/api/internal/ingest/${intakeId}/complete`,
    {
      method: 'POST',
      headers: {
...
async function createAcceptedDir(
  ...
  await Promise.all(
    Object.entries(files).map(([name, content]) =>
      fs.writeFile(path.join(acceptedDir, name), content),
    ),
  );
  return acceptedDir;
}
...
describe('admin ingest removal', () => {
  ...
  beforeEach(async () => {
    ...
    mocks.uploadsDir = await fs.mkdtemp(
      path.join(os.tmpdir(), 'besedy-remove-'),
    );

    const permissions = await import('@/lib/auth/permissions');
    (permissions.requireAuth as ReturnType<typeof vi.fn>).mockResolvedValue(
      'admin-1',
    );
  ...
  it('uses files-only cleanup when cancellation interrupted acceptance before the sidecar', async () => {
    ...
    prisma.recordingIntake.findUniqueOrThrow.mockResolvedValue(
      row({ status: 'REMOVED', audioHash: null }),
    );
    ...
    await expect(fs.stat(acceptedDir)).rejects.toThrow();
  });

  it('leaves a malformed accepted sidecar untouched after cancellation', async () => {
    const acceptedDir = await createAcceptedDir({
      'talk.mp3': 'audio',
      'talk.mp3.audiohash': 'not-a-valid-sidecar',
    });
    prisma.recordingIntake.findUnique.mockResolvedValue(
      row({
        status: 'CANCELLED',
  ...
  it('leaves ambiguous accepted contents untouched after cancellation', async () => {
    const acceptedDir = await createAcceptedDir({
      'talk.mp3': 'audio',
      'other.mp3': 'other audio',
      'talk.mp3.audiohash': `${HASH}  talk.mp3\n`,
    });
    prisma.recordingIntake.findUnique.mockResolvedValue(
      row({
  ...
  it('refuses to remove active intakes and leaves state untouched when the jobs API is down', async () => {
    ...
    const active = await removeIntake(
      adminRequest(`/api/admin/ingest/${INTAKE_ID}/remove`),
      {
        params: Promise.resolve({ intakeId: INTAKE_ID }),
      },
  ...
  it('keeps a removal active when web-owned cleanup fails so the worker can retry', async () => {
    prisma.recordingIntake.findUnique.mockResolvedValue(
      row({ status: 'REMOVING', jobId: REMOVAL_JOB_ID }),
    );
    removeRecordingWebState.mockRejectedValue(
  ...
  it('preserves the hash when the removal worker reports failure', async () => {
    prisma.recordingIntake.findUnique.mockResolvedValue(
      row({ status: 'REMOVING', jobId: REMOVAL_JOB_ID }),
    );
    ...
    expect(prisma.recordingIntake.update).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ status: 'FAILED', audioHash: HASH }),
      }),
    );
  });
});
