...
async function writeChunk(
  ...
  await fs.writeFile(path.join(chunksDir, `${index}.part`), contents);
}
...
describe('admin ingest routes', () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    mocks.uploadsDir = await fs.mkdtemp(
      path.join(os.tmpdir(), 'besedy-ingest-'),
    );
    ...
    logContentEvent = (await import('@/lib/audit/logger'))
      .logContentEvent as ReturnType<typeof vi.fn>;
    syncCatalogGroup = (await import('@/lib/catalog-sync'))
      .syncCatalogGroup as ReturnType<typeof vi.fn>;
  ...
  it('rejects non-admins on every route', async () => {
    ...
    const responses = await Promise.all([
      ...
      abortUpload(
        mutationRequest(`/api/admin/ingest/uploads/${INTAKE_ID}`, {
          method: 'DELETE',
        }),
        {
          params: Promise.resolve({ intakeId: INTAKE_ID }),
  ...
  it('creates an upload with an immutable-chunk staging directory', async () => {
    ...
    await expect(
      fs.stat(path.join(incomingDir, 'source.mp3')),
    ).rejects.toThrow();
    expect(
      (await fs.stat(path.join(incomingDir, '.chunks'))).isDirectory(),
    ).toBe(true);
    ...
    for (const dir of [
      ...
      path.join(mocks.uploadsDir, CATALOG_ID, 'incoming', INTAKE_ID, '.chunks'),
    ]) {
      expect((await fs.stat(dir)).mode & 0o2777).toBe(0o2770);
    }
  });
  ...
  it('publishes sequential immutable chunks and rejects out-of-order or oversized ones', async () => {
    ...
    expect(await fs.readFile(path.join(dir, '.chunks', '0.part'), 'utf8')).toBe(
      'hello',
    );
    expect(prisma.recordingIntake.updateMany).toHaveBeenCalledWith({
      ...
      data: {
        ...
        receivedBytes: { increment: BigInt(5) },
      },
    });
    ...
    await expect(
      fs.stat(path.join(dir, '.chunks', '1.part')),
    ).rejects.toThrow();
  });

  it('leaves a published chunk recoverable when the database commit loses a race', async () => {
    const dir = path.join(mocks.uploadsDir, CATALOG_ID, 'incoming', INTAKE_ID);
    await fs.mkdir(path.join(dir, '.chunks'), { recursive: true });
    prisma.recordingIntake.findUnique
      .mockResolvedValueOnce(
        baseRow({ receivedBytes: BigInt(5), receivedChunks: 1 }),
      )
      .mockResolvedValueOnce(
        baseRow({ receivedBytes: BigInt(10), receivedChunks: 2 }),
      );
    ...
    expect(await fs.readFile(path.join(dir, '.chunks', '1.part'), 'utf8')).toBe(
      'world',
    );
  ...
  it('finalizes a complete upload by submitting the ingest job', async () => {
    const dir = path.join(mocks.uploadsDir, CATALOG_ID, 'incoming', INTAKE_ID);
    await writeChunk(dir, 0, '01234');
    await writeChunk(dir, 1, '56789');

    const complete = baseRow({ receivedBytes: BigInt(10), receivedChunks: 2 });
    ...
    expect(fetchJobsApi).toHaveBeenCalledWith(
      `/catalogs/${CATALOG_ID}/ingest/jobs`,
      {
        method: 'POST',
        body: {
          intakeId: INTAKE_ID,
    ...
    expect(logContentEvent).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        resourceId: INTAKE_ID,
      }),
    );
    expect(await fs.readFile(path.join(dir, 'source.mp3'), 'utf8')).toBe(
      '0123456789',
    );
  });
  ...
  it('refuses to finalize an incomplete upload', async () => {
    prisma.recordingIntake.findUnique.mockResolvedValue(
      baseRow({ receivedBytes: BigInt(4) }),
    );
  ...
  it('keeps the upload for retry when the jobs API is unreachable or misconfigured', async () => {
    ...
    const complete = baseRow({ receivedBytes: BigInt(10), receivedChunks: 1 });
    prisma.recordingIntake.findUnique.mockResolvedValue(complete);
    const { JobsApiError, JobsApiConfigurationError } =
      await import('@/lib/jobs-api/server');
  ...
  it('lists intakes and reconciles terminal Prefect states', async () => {
    ...
    const response = await listIntakes(
      new NextRequest(
        `http://localhost/api/admin/ingest?catalogId=${CATALOG_ID}&limit=5`,
      ),
    );
  ...
  it('recovers an outcome the worker could not deliver and syncs on success', async () => {
    ...
    const response = await listIntakes(
      new NextRequest('http://localhost/api/admin/ingest'),
    );
  ...
  it('parses undelivered outcomes only when the marker and JSON are present', () => {
    ...
    expect(
      parseUndeliveredOutcome('completion_report_failed:not json'),
    ).toBeNull();
    expect(
    ...
    expect(
      parseUndeliveredOutcome('completion_report_failed:{"status":"WHAT"}'),
    ).toBeNull();
  });
  ...
  it('aborts only uploads that were not submitted', async () => {
    ...
    expect(prisma.recordingIntake.delete).toHaveBeenCalledWith({
      where: { id: INTAKE_ID },
    });
    await expect(fs.stat(dir)).rejects.toThrow();
  });
