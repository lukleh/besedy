...
-- Postgres 17+ catalogues NOT NULL as a named constraint, generated from the
-- table name at CREATE TIME; RENAME TABLE does not rename these, so they are
-- stale unless renamed explicitly. No app or Prisma-visible effect either way,
-- but left alone they are the last place "poster" survives in the catalog.
...
UPDATE "audit_log" SET "details" = replace(replace("details"::text, '"posterId"', '"artworkId"'),
  '"event_poster"', '"event_artwork"')::jsonb
WHERE "details"::text LIKE '%"posterId"%' OR "details"::text LIKE '%"event_poster"%';
...
BEGIN
  ...
  OR EXISTS (SELECT 1 FROM "audit_log" WHERE "resource" = 'event_poster' OR "subject_type" = 'event_poster') THEN
