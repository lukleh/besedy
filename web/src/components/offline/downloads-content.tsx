...
import { buildEventPagePath, buildRecordingPagePath } from "@/lib/api/recording-urls";
...
function recordHref(record: DownloadRecord): string {
  return record.event
    ? buildEventPagePath(record.catalogId, record.event.id)
...
export function DownloadsContent() {
  ...
  const { records, supported, hydrated, storage, activeKey } = useDownloadManager();
  ...
  const groups = useMemo(() => {
    ...
    return Array.from(byCatalog.entries()).map(([catalogId, items]) => ({
      catalogId,
      label: catalogs?.find((catalog) => catalog.id === catalogId)?.label ?? null,
      items,
    }));
  }, [records, catalogs]);

  const completeCount = records.filter((record) => record.status === "complete").length;
  const totalBytes = records.reduce((sum, record) => sum + (record.totalBytes || 0), 0);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
          ...
          <div className="flex items-center gap-2 text-muted-foreground">
            ...
            <span data-testid="downloads-storage">
              {storage && storage.quota > 0
                ? t("storageUsed", {
                    used: formatBytes(Math.max(storage.usage, totalBytes), locale),
      ...
      {!hydrated ? (
        ...
        supported && (
          <div className="rounded-lg border p-8 text-center" data-testid="downloads-empty">
            <Download className="mx-auto mb-3 h-10 w-10 text-muted-foreground" aria-hidden="true" />
          ...
          {groups.map((group) => (
            <section key={group.catalogId} className="space-y-3">
              <h2 className="text-sm font-medium text-muted-foreground">
                {group.label ?? t("unknownCatalog", { id: group.catalogId })}
              </h2>
      ...
      <AlertDialog open={confirmRemoveAll} onOpenChange={setConfirmRemoveAll}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{t("removeAllDescription")}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function DownloadCard({ record, isActive, locale }: DownloadCardProps) {
  const t = useTranslations("downloads");
  ...
  const formattedDate =
    dateYear !== null
      ? formatPartialDate(dateYear, dateMonth, dateDay, locale) ?? String(dateYear)
      : null;
  ...
  return (
    <Card data-testid={`download-card-${record.hash}`} data-status={record.status}>
      <CardContent className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center">
        <div className="min-w-0 flex-1 space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant="outline">{event ? t("eventLabel") : t("recordingLabel")}</Badge>
            {event && event.sessionIndex > 1 && (
              <Badge variant="outline">{t("sessionLabel", { index: event.sessionIndex })}</Badge>
            )}
            {record.status === "complete" && (
              <Badge variant="secondary">
                {record.transcriptBackend ? t("withTranscript") : t("audioOnly")}
              </Badge>
            )}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
            ...
            {record.error && <span className="text-destructive">{record.error}</span>}
          </div>
