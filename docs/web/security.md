...
plus `besedy-egress-watch.service`, which reconciles immediately after Docker
network create/destroy events. `besedy-egress-refresh.timer` also verifies every
5 minutes as a backstop in case an event is missed.
...
restart dockerd, which is why the event watcher and timer exist in addition to
the boot-time unit.
...
Reconciliation is serialized with a host lock, and every iptables command waits
for the xtables lock. This prevents deploy, event-watcher, and timer runs from
racing while replacing rules.
